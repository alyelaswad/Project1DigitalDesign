...
void CircuitReader::accessCirFile(std::string pathname)
{
    ...
    while (getline(cir_file, line))
    {
        ...
        // Compute total delay for the gate
         int max_input_delay = 0;
        for (const auto& input_name : gate_i.inputs)
        {
            for (const auto& input_gate : cir_gates)
            {
                if (input_gate.output == input_name)
                {
                    max_input_delay = max(max_input_delay, input_gate.delayofgate);
                    break;
                }
            }
        }

        // Assign total delay to the gate
        gate_i.delayofgate += max_input_delay;

        // Store the gate with its computed delay
